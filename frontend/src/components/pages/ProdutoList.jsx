...
const handleView = (produto) => navigate(`/produto/view/${getItemId(produto)}`); // Navega para a página de visualização do produto
const handleEdit = (produto) => navigate(`/produto/edit/${getItemId(produto)}`); // Navega para a página de edição do produto
...
const produtoId = getItemId(produto);
...
const updatedProdutos = produtos.filter((p) => getItemId(p) !== produtoId);
...
<TableRow key={getItemId(produto)} hover>
{columns.map((column, index) => {
if (column.field === 'id') return <TableCell key={index}>{getItemId(produto)}</TableCell>;
...
<Card key={getItemId(produto)} sx={{ mb: 2, elevation: 2 }}>
<CardContent sx={{ p: 2 }}>
